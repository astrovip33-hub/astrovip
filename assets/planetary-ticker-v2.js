(function(){
  ...
  function integrateNatalCalculator(){
    const menu=document.querySelector('.menu');
    if(menu&&!menu.querySelector('a[href="/harta-mea/"]')){const servicesLink=menu.querySelector('a[href="#servicii"]');const link=document.createElement('a');link.href='/harta-mea/';link.textContent='Harta mea';if(servicesLink)servicesLink.insertAdjacentElement('afterend',link);else menu.prepend(link)}
    const services=document.getElementById('servicii');if(!services||document.getElementById('harta-mea-cta'))return;
    const section=document.createElement('section');section.id='harta-mea-cta';section.className='natal-home-cta';section.innerHTML=`<div class="wrap"><div class="natal-home-card"><div><div class="kicker">Instrument gratuit AstroVip</div><h2>Calculează harta ta natală</h2><p>Vezi pozițiile planetelor, gradele exacte, retrogradarea și aspectele majore într-o hartă vizuală interactivă.</p></div><a class="cta" href="/harta-mea/">Deschide Harta mea</a></div></div>`;services.parentNode.insertBefore(section,services);
  }
  ...
  function boot(){applyPageTuning();integrateNatalCalculator();integrateForumLink();let tries=0;const timer=setInterval(()=>{tries++;if(window.Astronomy&&typeof Astronomy.GeoVector==='function'&&typeof Astronomy.Ecliptic==='function'){clearInterval(timer);render();setInterval(render,60000)}else if(tries>80){clearInterval(timer);document.querySelectorAll('[data-planet-ticker]').forEach(el=>el.textContent='Pozițiile planetare live nu s-au putut încărca.')}},150)}
