(function(){
  ...
  function applyPageTuning(){
    ...
    #servicii .grid .card:nth-child(2) img,#servicii .grid .card:nth-child(4) img,#servicii .grid .card:nth-child(6) img{aspect-ratio:1/1!important;object-fit:contain!important;object-position:center center!important;padding:10px!important;background:linear-gradient(180deg,rgba(7,18,34,.96),rgba(5,12,24,.98))!important}
    ...
    @media(max-width:820px){.brand{gap:8px!important}.brand::before{width:36px;height:36px}#servicii .grid .card:nth-child(2) img,#servicii .grid .card:nth-child(4) img,#servicii .grid .card:nth-child(6) img{padding:8px!important;aspect-ratio:1/1!important}.planet-item{font-size:17px!important}.planet-glyph{font-size:22px!important}.planet-track{gap:19px!important}.natal-home-cta{padding:18px 0 8px!important}.natal-home-card{padding:18px;flex-direction:column;align-items:stretch;text-align:center;border-radius:20px}.natal-home-card .cta{width:100%}}`;
  ...
  function boot(){applyPageTuning();integrateBrandMark();integrateAstroTools();integrateSecondTicker();integrateForumLink();let tries=0;const timer=setInterval(()=>{tries++;if(window.Astronomy&&typeof Astronomy.GeoVector==='function'&&typeof Astronomy.Ecliptic==='function'){clearInterval(timer);render();setInterval(render,60000)}else if(tries>80){clearInterval(timer);document.querySelectorAll('[data-planet-ticker]').forEach(el=>el.textContent='Pozițiile planetare live nu s-au putut încărca.')}},150)}
