(function(){
  ...
  function applyPageTuning(){
    ...
    @media(min-width:821px){body:not(.guide-page) .hero{background-image:linear-gradient(90deg,rgba(3,15,33,.99) 0%,rgba(3,15,33,.94) 34%,rgba(3,15,33,.70) 50%,rgba(3,15,33,.26) 64%,rgba(3,15,33,.04) 78%),url('/assets/1000043152.png')!important;background-position:center,82% center!important;background-size:auto,cover!important;background-repeat:no-repeat!important}body:not(.guide-page) .hero:before{inset:0 54% 0 0!important;background:linear-gradient(90deg,#041126 0%,rgba(4,17,38,.98) 58%,rgba(4,17,38,.56) 82%,rgba(4,17,38,0) 100%)!important}body:not(.guide-page) .hero:after{height:120px!important;background:linear-gradient(180deg,transparent,rgba(4,17,38,.72))!important}
      body:not(.guide-page) .trust-icons-only{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:14px!important;align-items:stretch!important}
      body:not(.guide-page) .trust-icon-only,body:not(.guide-page) .trust-commerce-card{height:276px!important;min-height:276px!important;border-radius:22px!important;border:1px solid rgba(114,216,255,.46)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.16),0 18px 42px rgba(0,0,0,.34),0 0 28px rgba(57,191,255,.08)!important;overflow:hidden!important}
      body:not(.guide-page) .trust-icon-only{position:relative!important;display:grid!important;grid-template-rows:128px 1fr!important;padding:0!important;background:linear-gradient(160deg,#06152d 0%,#07366c 58%,#05203f 100%)!important;text-align:center!important;text-decoration:none!important}
      body:not(.guide-page) .trust-icon-only:hover{transform:translateY(-4px)!important;border-color:rgba(56,245,138,.66)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.18),0 24px 52px rgba(0,0,0,.42),0 0 34px rgba(56,245,138,.12)!important}
      body:not(.guide-page) .trust-icon-only img{grid-row:1!important;width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;object-fit:contain!important;object-position:center!important;margin:0!important;padding:14px 22px!important;background:linear-gradient(180deg,#fff 0%,#f7f9fc 100%)!important;transform:none!important}
      body:not(.guide-page) .trust-card-copy{grid-row:2!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important;gap:5px!important;padding:14px 18px 17px!important;background:linear-gradient(180deg,rgba(4,17,38,.18),rgba(4,17,38,.72))!important}
      body:not(.guide-page) .trust-card-kicker{font-family:Inter,ui-sans-serif,system-ui,sans-serif!important;color:#8ee9ff!important;font-size:11px!important;line-height:1.15!important;font-weight:900!important;letter-spacing:1.35px!important;text-transform:uppercase!important}
      body:not(.guide-page) .trust-card-title{font-family:ui-serif,Georgia,'Times New Roman',serif!important;color:#f8fbff!important;font-size:clamp(24px,2vw,31px)!important;line-height:1.02!important;font-weight:800!important;letter-spacing:-.35px!important;text-shadow:0 0 18px rgba(255,255,255,.08)!important}
      body:not(.guide-page) .trust-card-subtitle{max-width:30ch!important;color:#c9d8ef!important;font-size:12px!important;line-height:1.32!important;font-weight:700!important}
      body:not(.guide-page) .trust-card-link{margin-top:3px!important;color:#70ffad!important;font-size:11px!important;font-weight:900!important;letter-spacing:.35px!important;text-transform:uppercase!important}
      body:not(.guide-page) .trust-commerce-card{display:flex!important;flex-direction:column!important;justify-content:center!important;align-items:center!important;padding:22px!important;background:radial-gradient(circle at 78% 12%,rgba(57,191,255,.24),transparent 28%),linear-gradient(145deg,#0750a0 0%,#078fc3 55%,#087b9c 100%)!important}
      body:not(.guide-page) .trust-commerce-card strong{font-family:ui-serif,Georgia,'Times New Roman',serif!important;font-size:clamp(31px,2.6vw,42px)!important;letter-spacing:-.5px!important;text-shadow:0 2px 18px rgba(0,0,0,.22)!important}
    }
    @media(max-width:820px){.brand{gap:8px!important}.brand::before{width:36px;height:36px}.planet-item{font-size:17px!important}.planet-glyph{font-size:22px!important}.planet-track{gap:19px!important}.natal-home-cta{padding:18px 0 8px!important}.natal-home-card{padding:18px;flex-direction:column;align-items:stretch;text-align:center;border-radius:20px}.natal-home-card .cta{width:100%}#astrovip-legal-identity{margin-top:18px;padding:16px 13px;border-radius:15px}.legal-identity-meta{display:grid;gap:5px}.legal-identity-meta span{white-space:normal;overflow-wrap:anywhere}}`;
  ...
  function enhanceTrustCards(){
    const cards=document.querySelectorAll('.trust-icons-only .trust-icon-only');
    if(cards.length<2)return;
    const entries=[
      {card:cards[0],kicker:'Protecția consumatorilor',title:'ANPC',subtitle:'Autoritatea Națională pentru Protecția Consumatorilor',link:'Site oficial ↗'},
      {card:cards[cards.length-1],kicker:'Etică profesională',title:'Codul Etic al Astrologului',subtitle:'Asociația Astrologilor din România · AAR',link:'Consultă codul etic ↗'}
    ];
    entries.forEach(({card,kicker,title,subtitle,link})=>{
      if(card.querySelector('.trust-card-copy'))return;
      const copy=document.createElement('span');
      copy.className='trust-card-copy';
      copy.innerHTML=`<span class="trust-card-kicker">${kicker}</span><strong class="trust-card-title">${title}</strong><span class="trust-card-subtitle">${subtitle}</span><span class="trust-card-link">${link}</span>`;
  ...
  function boot(){applyPageTuning();integrateBrandMark();integrateAstroTools();integrateSecondTicker();integrateForumLink();enhanceTrustCards();integrateLegalIdentity();let tries=0;const timer=setInterval(()=>{tries++;if(window.Astronomy&&typeof Astronomy.GeoVector==='function'&&typeof Astronomy.Ecliptic==='function'){clearInterval(timer);render();setInterval(render,60000)}else if(tries>80){clearInterval(timer);document.querySelectorAll('[data-planet-ticker]').forEach(el=>el.textContent='Pozițiile planetare live nu s-au putut încărca.')}},150)}
