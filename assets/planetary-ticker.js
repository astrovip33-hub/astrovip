(function(){
  ...
  function calc(){
    ...
    BODIES.forEach(([body,label,glyph],i)=>{
      try{
        ...
        if(i>1){const next=geoLongitude(body,later);retro=dayDelta(lon,next)<-0.0005}
  ...
  function boot(){
    let tries=0;const t=setInterval(()=>{tries++;if(window.Astronomy&&typeof Astronomy.GeoVector==='function'&&typeof Astronomy.Ecliptic==='function'){clearInterval(t);render();setInterval(render,60000)}else if(tries>60){clearInterval(t);document.querySelectorAll('[data-planet-ticker]').forEach(el=>el.textContent='Pozițiile planetare live nu s-au putut încărca.')}},150);
  }
