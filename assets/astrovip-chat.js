...
const launcher=document.getElementById("avchat-launcher")||document.createElement("button");launcher.id=launcher.id||"avchat-launcher";launcher.classList.add("avchat-launcher");launcher.type="button";if(!launcher.querySelector(".avchat-launcher-label"))launcher.innerHTML='<span class="avchat-launcher-dot"></span><span class="avchat-launcher-label"></span>';
...
if(!launcher.isConnected)document.body.append(launcher);document.body.append(panel);
...
async function sendMsg(){const text=input.value.trim();if(!text||state.busy)return;state.busy=true;send.disabled=true;try{await ensureThread();const r=await post({action:"send_visitor",thread_id:state.thread,session_token:state.session,text,source_lang:state.lang});input.value="";resizeInput();showMsg(r.message);setStatus(currentCopy().online);body.scrollTop=body.scrollHeight}catch(e){setStatus(currentCopy().error,true)}finally{state.busy=false;send.disabled=false;input.focus()}}
function syncOpenState(){const open=panel.classList.contains("is-open");launcher.setAttribute("aria-expanded",open?"true":"false");document.body.classList.toggle("avchat-open",open&&window.matchMedia("(max-width:820px)").matches);if(open)alignPanel()}
launcher.onclick=async()=>{panel.classList.toggle("is-open");syncOpenState();if(panel.classList.contains("is-open")){paintCopy();alignLauncher();try{await ensureThread();await poll()}catch(e){setStatus(currentCopy().error,true)}setTimeout(()=>input.focus(),80)}};
$(".avchat-close").onclick=()=>{panel.classList.remove("is-open");syncOpenState()};
send.onclick=sendMsg;input.addEventListener("input",resizeInput);input.addEventListener("keydown",e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendMsg()}});document.addEventListener("keydown",e=>{if(e.key==="Escape"&&panel.classList.contains("is-open")){panel.classList.remove("is-open");syncOpenState();launcher.focus()}});
...
document.addEventListener("visibilitychange",()=>{if(!document.hidden&&state.thread){poll();schedulePoll(panel.classList.contains("is-open")?4000:12000)}});
})();